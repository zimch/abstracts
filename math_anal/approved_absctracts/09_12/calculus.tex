    ...
    \begin{proof}
        ...
        \begin{enumerate}
            \item $F(x, y) = (x, \Phi(x, y)) : D \rightarrow \R^{n + m}$
                ...
                \par $\hat{\hat U} \subset B_{\sqrt{2}\delta}^{n + m} \subset \hat{U}, \quad$ $n, \ m$ -- размерности
                \par Тогда $F(\hat{\hat U}) \subset F(\hat U) = \hat V$
                \par $(x_0, 0) \in F(\hat{\hat U}) \subset \hat V$
                \par $\exists \epsilon > 0 : B_\epsilon^{n+m}(x_0, 0) \subset F(\hat{\hat U})$ - так как $\hat V$ открыто
            ...
            \item $\Phi(x, \phi(x)) = 0$
                ...
                \par $\Phi'(x, \phi(x)) = \quad \begin{pmatrix}I \\ \phi'(x)\end{pmatrix} = \Phi'(x, \phi(x)) + \underbrace{\Phi'_y(x, \phi(x))}_{\text{обр.}} - \phi'(x)$ % ТУТ - У \phi??
        \end{enumerate}
    \end{proof}
