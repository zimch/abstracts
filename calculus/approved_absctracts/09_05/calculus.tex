...
\begin{document}
    ...
    % \pagebreak
    %\section*{Повторение}
    {\noindent\Large\textbf{Повторение}} \hfill \boxed{\textup{\textbf{5 сентября 2022}}}
    ...
    % Рассмотрим $f^{-1} \circ f = \id_D$  перенёс вниз
    ...
    \subsection*{Производная о локальном обращении}
    ...
    Домножим на $(f'(x_0))^{-1}$ справа: $(f^{-1})'(f(x_0)) = (f'(x_0))^{-1}$
    ...
    \begin{theorem}
        ...
        \[
            (f^{-1})'(f(x)) = (f'(x))^{-1}, \ \forall x \in U %\quad f'(x_0) \in I    
        \]
    \end{theorem}
