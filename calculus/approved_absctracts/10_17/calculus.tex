...
\begin{document}
    ...
    \begin{lemma} % тут нужно \overline сделать длинными% done
    ...
    \begin{proof}
        ...
        \]
        \[
            x \in \Int E \Leftrightarrow \exists \epsilon > 0 \ B_\epsilon(x) \subset E \Leftrightarrow \exists \delta > 0 : B_\delta\big(g(x)\big) \subset g(E) \Leftrightarrow   
        \]
        \[
        ...
        \[
            \bigcup_{l=1}^N C_l \subset E^{\frac{\delta}{2}} = \{x \in \R^n \ | \ \dist (x, E) < \delta/2\} \subset \overline {E^{\frac{\delta}{2}}} \subset G  
        \]
        \[
            \forall x_1, x_2 \in E^{\frac{\delta}{2}} \ \|g(x_1) - g(x_2)\| \le M \cdot \|x_1 - x_2\| \quad \max_{\overline{E^{\frac{\delta}{2}}}} \|g'\| = M   
        \]
        \[
            g(C_l) \subset B_{M \cdot \diam C_l/2} \big(g(x_l)\big) \subset \widetilde C_l, \ l(\widetilde C_l) = M \cdot \diam C_l    
        \]
        \[
            v(\widetilde C_l) = \big(l(\widetilde C_l)\big)^n = M^n \cdot (\diam C_l)^n = M^n \Big(\frac{\diam C_l}{\sqrt{n}}\Big)^n (\sqrt{n})^n = (M\sqrt{n})^n v (C_l)    
        \]
        \[
            g(E) \subset \bigcup_{l=1}^N \widetilde C_l, \quad \sum_{l=1}^N \widetilde C_l = (M\sqrt{n})^n \cdot \epsilon   
        \]
    \end{proof}

    \begin{remark}
        В условии теоремы $\exists \rhimani{G} f \circ g |\det g'|$
    \end{remark}
    % ТУТ ПЕРЕДЕЛАТЬ КРАСИВО
    \begin{proof}
        $\supp f = \overline{\{y \in g(G) \ | \ f(y) \not = 0\}}$
        \par $\supp (f \circ g |\det g'|) = \supp f \circ g = \overline{\{x \in G \ | \ (f \circ g) (x) \not= 0\}}$
        \par $g\big(\{x \in G \ | \ f \circ g \not = 0\}\big) = \{y \in g(G) \ | \ f(y) \not = 0\}$
        \par $\xRightarrow[\text{л. 16}]{}$ замыкание совпадает
        \par $ $
    ...
    \begin{lemma} % ЛЕММА 17
        ...
        \[
            \big((g_k)(x)\big)_i = x_i, \quad \forall i \not= k, \quad i \text{ -- координата}  
        \]
    \end{lemma}
    \begin{proof}
        Индукция
        ...
        \par Пусть $x_0 \in G \quad g'(x_0) = \begin{pmatrix}
            ...
            0 & I_{n-k}
        \end{pmatrix}$
        \par $0 \not= \det g'(x_0) = \det \frac{\partial g}{\partial y} (x_0) \Rightarrow$ не все миноры порядка $k-1$ нулевые
        \par \quad перенумерацией компонентов добъемся того, чтобы главный минор был $\not= 0$
        \[
            f : G \rightarrow \R^n \quad \big(f(x)\big)_i = \begin{cases}
                \big(g(x)\big)_i, \ i < k \\
                x_i, \ i \ge k
            \end{cases}    
        \]
        ...
        \par $u \in f(U)$
        \[
            i < k \quad \big(h(u)\big)_i = \big(g \circ f^{-1}(u)\big)_i = \big(f \circ f^{-1}(u)\big)_i = u_1
        \]
        \[
            i \ge k \quad \big(h(u)\big)_i = \big(g \circ f^{-1}(u)\big)_i =\big(f^{-1}(u)\big)_i = u_i    
        \]
        \[
    ...
    \begin{lemma}
        ...
        \begin{lemma}[14']
            ...
            \[
                \lrhimani{G} f \circ g |\det g'| = \lrhimani{g(G)} f,\quad \urhimani{G} f \circ g |\det g'| = \urhimani{g(G)} f    
            \]
        \end{lemma}
    \end{lemma}
    ...
    \begin{proof}
        ...
        \]
        \[
            g^{-1}\big([x_i - \epsilon_{x_i}, x_i + \epsilon_{x_i}]\big) \text{ -- отрезок} \Rightarrow \text{ достаточно доказать:}    
        \]
        \[
        ...
        % ТУТ Я УСНУЛ
        $P_y$ - разбиение $g\big([a,b]\big), \quad P_x = g^{-1}(P_y)'' = \{ g^{-1}(\pi) \mid \pi \in P_y\}$

        $\min_{g[a,b]} | (g^{-1})'| \cdot d(P_y) \le d(P_x) \le \max_{g([a,b])} | (g^{-1})'| \cdot d(P_y)$
        
        $\sum_{\pi_y \in P_y}\sup_{\pi_y}f \cdot v(\pi_y) = 
        \sum_{\pi_x \in P_x}\sup_{\pi_x} (f \circ g) \cdot |g'\big(\xi(\pi_x)\big)| \cdot v(\pi_x)$

        $\pi_y = g(\pi_x)$   

        $v(\pi_y) = |g(\beta) - g(\alpha)| = |g'(\xi)| \cdot (\beta\alpha)$

        прадалжаем неравенство (емае неправильное все стёр....) 
        
        $\displaystyle\le \underbrace{\sup_{[a,b]}|g'|}_{<\infty} \sum_{\pi_x \in P_x}\sup_{\Pi_x}f\circ g \cdot v(\pi_x)$

        $P_{x,k}:d(P_{x,k}) \to 0, k \to \infty$

        $P_{y,k}:d(P_{y,k}) \to 0, k \to \infty$

        $\forall \pi_x \ \exists \xi(\pi_x) < \pi_x \quad v(\pi_y) = |g'\big(\xi(\pi_x)\big)|\cdot v(\pi_x)$
        ...
        $\implies \overline\int_{g([a,b])}f = \overline\int_{[a,b]}f\circ g |\det g'|$
    \end{proof}
